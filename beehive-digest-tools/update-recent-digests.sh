...
function compile() {
  ...
  python list-keys-v1 --end today --periods 3 | python list-datasets-v1 | python export-recent-datasets-v1 --since $WINDOW -p 8 /storage/datasets/v1

  # TODO figure out how to handle sdf / beehive plugins
  # TODO look into what the -p support is
  python list-keys-v2 --end today --periods 3 | python list-datasets-v2 | python export-recent-datasets-v2 --since $WINDOW /storage/datasets/v2 $PWD/sdf.csv /storage/plugins/*.plugin
